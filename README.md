...
入力内容は **入力から約1.2秒後に自動保存**され、**ウィンドウを閉じるとき**にも保存されます。

- **メイン設定:** `%AppData%\YomiVox\settings.json`（VOICEVOX の URL、Twitch の OAuth / Client Secret、再生デバイス、オプションのキャラ設定など）
- **視聴者まわり（チャット由来）:** `%AppData%\YomiVox\viewer_settings.json`（`!readname` の読み、`!voice` の個人合成、ユーザーごとの話者・スタイル割当）

初回起動時、以前のバージョンで `settings.json` にだけ入っていた上記の視聴者データは、自動で `viewer_settings.json` に移されます。

エクスプローラーのアドレスバーに `%AppData%\YomiVox` と入れるとフォルダが開けます。OAuth トークンや Client Secret は **メインの `settings.json`** に平文で入るので、PC を共有する場合は注意してください。
...
- `!voice 四国めたん` / `!voice 1` … **話者キャラ**を変更（番号は `!voice list` の並び・1 始まり）
- `!voice list` … 利用可能な**話者キャラ**の一覧
- `!style ノーマル` / `!style ささやき` / `!style 1` … **いまのキャラ**の**スタイル**を変更（`!style list` の並び）
- `!style list` … 自分に割り当てのキャラの**スタイル**一覧（`!よみ …` は `!style` と同じ）
- `!voice help` / `!voice ?` … チャットにコマンド一覧を表示

### 合成パラメータ（個人・チャット）

話速・音高・抑揚・音量を **Twitch ログイン名ごと**に上書きします（`viewer_settings.json` に保存）。**オプションの「キャラクター」タブ**で設定したキャラ別の値より、こちらが**優先**されます。
...
- 各 Twitch ユーザーには **ランダムな話者スタイル**が割り当てられ、割当は **`viewer_settings.json` に保存**されるため、次回起動後も同じ声で読み上げます。
